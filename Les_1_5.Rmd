...
Задача выполняется на данных: Яндекс.Маркет, смартфоны (Москва)
Загрузка библиотек.
```{r}
library('RCurl')
...
html2 <- getURL(fileUR2L, .encoding = 'UTF-8')
doc2 <- htmlParse(html2, encoding = "UTF-8")
rootNode2 <- xmlRoot(doc2)
```
Выбирем наименования моделей и объединяем данные с двух страниц
```{r}
...
Rat <- c(r,r2)
```
Выбирем версию андройда из общех характеристик 
```{r}
...
tr <- c(t,t2)
```
Выбирем остальные характеристики
```{r}
